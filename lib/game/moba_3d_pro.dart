...
class _Moba3DProState extends State<Moba3DPro> {
  ...
  final enemies=<ProUnit>[],minions=<ProUnit>[],jungle=<ProUnit>[];
  ...
  void _buildJungle(){
    for(final p in [three.Vector3(-25,0,-17),three.Vector3(25,0,17),three.Vector3(-25,0,17),three.Vector3(25,0,-17)]){
      final m=ProUnit('C'+jungle.length,2,p,CombatStats(maxHp:700,attack:25,armor:12,magicResist:10,moveSpeed:0,range:5));m.mesh=_monster(false);jungle.add(m);view.scene.add(m.mesh!);
    }
    final t=ProUnit('TURTLE',2,three.Vector3(0,0,0),CombatStats(maxHp:4500,attack:65,armor:30,magicResist:25,moveSpeed:0,range:8));t.mesh=_monster(true);jungle.add(t);view.scene.add(t.mesh!);
    final d=ProUnit('DRAKE',2,three.Vector3(0,0,46),CombatStats(maxHp:6500,attack:90,armor:35,magicResist:30,moveSpeed:0,range:10));d.mesh=_monster(true);jungle.add(d);view.scene.add(d.mesh!);
  }
  ...
  void _addMinion(int team,double z,double x,bool ranged){final m=ProUnit('M'+minions.length,team,three.Vector3(x,0,z),CombatStats(maxHp:ranged?260:340,attack:ranged?32:38,armor:8,magicResist:8,moveSpeed:team==0?6.2:5.9,range:ranged?15:4));m.lane=z;m.mesh=_minion(team==0,ranged);minions.add(m);view.scene.add(m.mesh!);}
  void _tick(double dt){
    ...
    _playerUpdate(dt);_enemyUpdate(dt);_minionUpdate(dt);_jungleUpdate(dt);_towerUpdate(dt);_cleanup();
  ...
  void _minionUpdate(double dt){
    for(final m in minions)if(m.alive){ProUnit? target;double best=m.stats.range+2;for(final o in minions)if(o.alive&&o.team!=m.team&&(o.position.z-m.position.z).abs()<7){final d=_dist(m.position,o.position);if(d<best){best=d;target=o;}}if(target!=null){m.attackTimer-=dt;if(m.attackTimer<=0){m.attackTimer=.8;target.takeDamage(m.stats.attack,MobaDamageType.physical);}}else{m.position.x+=(m.team==0?1:-1)*m.stats.moveSpeed*dt;m.mesh?.position.setValues(m.position.x,0,m.position.z);}}}
  void _jungleUpdate(double dt){
    for(final j in jungle){if(!j.alive)continue;final d=_dist(j.position,player.position);if(d<j.stats.range+6){j.attackTimer-=dt;if(j.attackTimer<=0){j.attackTimer=1.1;_damagePlayer(j.stats.attack);}}}
  }
  ...
  void basicAttack(){if(!player.alive)return;ProUnit? target;double best=player.stats.range+2;for(final e in enemies)if(e.alive){final d=_dist(player.position,e.position);if(d<best){best=d;target=e;}}for(final m in minions)if(m.alive&&m.team==1){final d=_dist(player.position,m.position);if(d<best){best=d;target=m;}}if(target!=null){target.takeDamage(player.stats.attack,MobaDamageType.physical);if(!target.alive){if(target.id.startsWith('E')){kills++;gold+=180;level=math.min(15,1+kills~/2);}else gold+=45;}}}
  ...
  Widget _hud()=>Row(children:[_pill('TIME '+time.toInt().toString()),const SizedBox(width:6),_pill('K/D '+kills.toString()+'/'+deaths.toString()),const SizedBox(width:6),_pill('LV '+level.toString()+'  GOLD '+gold.toInt().toString()),const Spacer(),_pill('WAVE '+wave.toString())]);
