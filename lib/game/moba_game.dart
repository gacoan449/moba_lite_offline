...
class MOBAOfflineGame extends FlameGame with HasCollisionDetection {
  ...
  @override Future<void> onLoad() async {
    ...
    world.add(const ArenaMapComponent());
    ...
    attackButton = HudButtonComponent(
      ...
      margin: const EdgeInsets.only(right: 28, bottom: 30), onPressed: () => player.basicAttack(isPremiumWeapon));
    ...
    skillButton = HudButtonComponent(
      ...
      margin: const EdgeInsets.only(right: 115, bottom: 36), onPressed: player.useSkill);
    camera.viewport.add(skillButton);
    buildBattlefield();
    overlays.add('HUD');
  }
  void applyHeroStats() {
    player.maxHp = selectedHero.hp.toDouble(); player.hp = player.maxHp; player.speed = selectedHero.speed.toDouble();
    player.baseDamage = selectedHero.damage.toDouble(); player.baseSkillDamage = selectedHero.skillDamage.toDouble();
  }
  void buildBattlefield() {
    for (final turret in List<TurretComponent>.from(turrets)) turret.removeFromParent();
    for (final monster in List<JungleMonster>.from(monsters)) monster.removeFromParent();
    for (final enemy in List<BotEnemyComponent>.from(enemies)) enemy.removeFromParent();
    for (final minion in List<MinionComponent>.from(minions)) minion.removeFromParent();
    turrets.clear(); monsters.clear(); enemies.clear(); minions.clear();
    const lanes = [-560.0, 0.0, 560.0];
    for (final y in lanes) {
      for (final x in [-2050.0, -1250.0]) { final t = TurretComponent(position: Vector2(x, y), allied: true, lane: 'lane'); turrets.add(t); world.add(t); }
      for (final x in [1250.0, 2050.0]) { final t = TurretComponent(position: Vector2(x, y), allied: false, lane: 'lane'); turrets.add(t); world.add(t); }
    }
    for (final p in <Vector2>[Vector2(-950, -300), Vector2(-950, 300), Vector2(0, -900), Vector2(0, 900)]) { final m = JungleMonster(position: p); monsters.add(m); world.add(m); }
    final boss = JungleMonster(position: Vector2.zero(), boss: true); monsters.add(boss); world.add(boss);
    spawnWave(); spawnEnemyHeroes();
  }
  void spawnEnemyHeroes() {
    for (final p in <Vector2>[Vector2(1800, -560), Vector2(1800, 0), Vector2(1800, 560), Vector2(1800, -120), Vector2(1800, 120)]) { final e = BotEnemyComponent(p.clone()); enemies.add(e); world.add(e); }
  }
  void spawnWave() {
    const lanes = [-560.0, 0.0, 560.0];
    for (final y in lanes) { for (int i = 0; i < 3; i++) {
      ...
      minions.addAll([a, e]); world.add(a); world.add(e);
    }}
    quest = '3 LANE • MINION WAVE • HANCURKAN TURRET';
  }
  @override void update(double dt) {
    super.update(dt); enemies.removeWhere((e) => e.isRemoved); minions.removeWhere((e) => e.isRemoved); turrets.removeWhere((e) => e.isRemoved); monsters.removeWhere((e) => e.isRemoved);
    messageTimer = max(0, messageTimer - dt).toDouble(); hudTick.value++;
  ...
  void onTurretDestroyed(TurretComponent turret) { gold += 180; xp += 120; flashMessage('TURRET MUSUH HANCUR! +180 GOLD'); }
  void onMonsterKilled(JungleMonster monster) { gold += monster.boss ? 500 : 80; xp += monster.boss ? 350 : 70; flashMessage(monster.boss ? 'BOSS JUNGLE DIKALAHKAN! +500 GOLD' : 'MONSTER JUNGLE DIKALAHKAN! +80 GOLD'); }
  ...
  Future<void> selectHero(HeroDefinition hero) async { if (!ownedHeroes.contains(hero.id)) return; selectedHeroId = hero.id; await _prefs?.setString('selected_hero', hero.id); applyHeroStats(); hudTick.value++; }
  Future<bool> buyHero(HeroDefinition hero) async { if (ownedHeroes.contains(hero.id)) return true; if (coins < hero.price) return false; coins -= hero.price; ownedHeroes.add(hero.id); await _saveWallet(); await selectHero(hero); return true; }
  Future<bool> buySkin(Map<String, Object> skin) async { final id = skin['id']! as String; final hero = skin['hero']! as String; final price = skin['price']! as int; if (!ownedHeroes.contains(hero) || ownedSkins.contains(id) || coins < price) return false; coins -= price; ownedSkins.add(id); await equipSkin(skin); await _saveWallet(); return true; }
  Future<void> equipSkin(Map<String, Object> skin) async { final id = skin['id']! as String; if (!ownedSkins.contains(id)) return; selectedSkinId = id; await _prefs?.setString('selected_skin', id); hudTick.value++; }
  ...
  @override void render(Canvas canvas) { canvas.drawRect(const Rect.fromLTWH(-3500, -1800, 7000, 3600), Paint()..color = const Color(0xff294b31)); super.render(canvas); }
}
