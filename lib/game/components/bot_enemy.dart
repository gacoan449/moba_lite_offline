...
class BotEnemyComponent extends PositionComponent
    with CollisionCallbacks, HasGameReference<MOBAOfflineGame> {
  double maxHp;
  double currentHp;
  final double speed;
  ...
  void render(Canvas canvas) {
    ...
    WorldRender3D.armoredBody(canvas, center: Offset(c.dx, c.dy + 5), width: 43, height: 43,
      primary: const Color(0xff7b1f2a), secondary: const Color(0xffe4545e), phase: visualTime);
    canvas.drawCircle(Offset(c.dx, c.dy - 18), 13, Paint()..color = const Color(0xffd94a54));
    canvas.drawPath(Path()..moveTo(c.dx - 12, c.dy - 18)..lineTo(c.dx, c.dy - 28)..lineTo(c.dx + 12, c.dy - 18)..lineTo(c.dx + 7, c.dy - 11)..lineTo(c.dx - 7, c.dy - 11)..close(), Paint()..color = const Color(0xff45121a));
    canvas.drawCircle(Offset(c.dx - 4, c.dy - 17), 2.4, Paint()..color = Colors.white.withValues(alpha: .8));
    canvas.drawCircle(Offset(c.dx + 4, c.dy - 17), 2.4, Paint()..color = Colors.white.withValues(alpha: .8));
    canvas.drawCircle(Offset(c.dx, c.dy + 7), 4 + pulse.abs() * .6, Paint()..color = Colors.redAccent.withValues(alpha: .8));
